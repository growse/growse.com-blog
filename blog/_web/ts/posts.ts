    ...
    fetch("/posts.json")
      ...
      .then((postList) => {
        ...
        if (document.location.pathname == "/") {
          document
            .querySelector("#articlenav > li > a")
            ?.classList.add(hereClass);
        }
        const postListElement = document.getElementById("postlist")!;
        const overlayScrollbars = OverlayScrollbars(
          postListElement,
          this.overlayScrollBarOptions,
        );
        ...
        if (percentageDown > 50) {
          const value =
            ...
            windowHeight / 2 +
            document
              .querySelector("nav#postlist ul li")
