...
export class Search {
    ...
    constructor() {
        ...
        if (searchDiv != null) {
            ...
            searchDiv.querySelector("input#searchbox")?.setAttribute("name", this.searchParamQueryStringName);
        }
    ...
    private search(searchTerm: string, page: number) {
        ...
        const template = document.querySelector("#searchResultTemplate")!! as HTMLTemplateElement;
        ...
        fetch(this.searchURL, {method: 'POST', body: params})
            ...
            .then(response => {
                ...
                document.querySelector("span#totalhits")!!.textContent = searchResults.totalHits.toLocaleString();
                searchResults.hits.forEach(hit => {
                    const populatedSearchResult = this.putDataIntoTheSearchResultTemplate(template.content, hit);
                    document.querySelector("#searchResultList")?.appendChild(populatedSearchResult);
                });
    ...
    private buildPager(pagerDivId: string, searchResults: SearchResults, page: number) {
        ...
        pager?.appendChild(this.buildPagerButton(page > 1 ? page - 1 : 1, "<", "Previous Page"));

        for (let ii = 1; ii <= pages; ii++) {
            pager?.appendChild(this.buildPagerButton(ii, ii.toString(), `page ${ii}`, ii == page ? ["current"] : ["absolutePage"]));
        }
        pager?.appendChild(this.buildPagerButton(page < pages ? page + 1 : pages, ">", "Next Page"));
        pager?.appendChild(this.buildPagerButton(pages, ">>", "Last Page"));
    }

    private buildPagerButton(page: number, text: string, title: string, classList?: string[]): HTMLElement {
    ...
    private putDataIntoTheSearchResultTemplate(template: DocumentFragment, hit: SearchHit) {
        ...
        template.querySelector("time")!!.setAttribute("datetime", publishedDate.toISOString());
        if (hit.fragments.content != null) {
            template.querySelector(".fragment")!!.innerHTML = hit.fragments.content.join("...");
        }
